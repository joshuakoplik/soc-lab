# Northwind Range — Phase 2 Build Specification (deferred)

> **Status: deferred, not being built yet.** Northwind's active build is the Phase 1 spec in
> `SPEC.md` — a RAG platform with no deliberate CVEs, measuring application-design weaknesses
> (prompt-only entitlements, confused-deputy tools, unreviewed ingestion) rather than
> infrastructure vulnerabilities. This document is what `SPEC.md` §11 ("Phase 2 hooks")
> refers to as the eventual infrastructure-vulnerability-chain tier (the `nw_ml` supply-chain
> pivot, CVE-pinned components, capture-the-flag scoring) — kept here as a reference for when
> that phase starts, not as current build instructions. Do not build against this file until
> Phase 1 is complete and Phase 2 is explicitly greenlit.
